export default [
  //CYG
  "Blague zoophile ? C'est un mec qui rentre dans un bar ...",
  "Blague zoophile ? C'est un mec qui prend son élan ...",
  "Y a 5 places dans une Mégane.",
  "Salto cul nu !",
  "Il est ricard moins quart.",
  "Problème de chaudière ...",
  //RDO
  "Pépite !",
  "Arrêtez de me limer !",
  "Nom de Zeus !",
  "Rio, à Barra Club, l'autoroute du cul je te dis !",
  "Missile !",
  "Tout en alcoolisme ...",
  "Carton !",
  "Capitaine Jachère est là pour vous.",
  //BEJ
  "Et ce soir ... on chope !",
  "OOOOOUUuuuuuwwwAAAAAAAAAAAAAAA",
  "J'en veux Jooooooooooe !",
  "Combien tu mesures ???? ... Jamais vu un tas de merde aussi haut !",
  "C'est ça que j'aime bien chez toi ... !",
  "Comme elle est belle !",
  "Tu vas passer au tribunal de la jachère",
  //BRM
  "Tu sais pas qui chui !",
  "Mais téma wesh !",
  "Ah !",
  "C'est troooooooop !",
  "DAMN !",
  "Nan mais pour moi ...",
  //DAS
  "On se retrouve en bas, on y va molo.",
  "Molo, on a dit !",
  "SSBU chez moi ce soir ?",
  "Pas ce soir c'est Shabbat.",
  "Je te fais un Revolut ?",
  "Tu me fais un Revolut",
  //LAAL
  "Oh, mais c'est Rafabelle !",
  "On dit pas qu'elle est bonne !",
  "Vous êtes trop des choux !",
  "Brondon !",
  "Je suis une star d'UX !",
  //BME
  "Jarrive dans une heure je suis à Nogent",
  "J'ai besoin de bitcher",
  "Pour tenir la croissance en CA, faut tenir la croissance en effectif et pour l'instant ça fait que baisser, donc pour l'instant ça pue. Car on doit faire plus de CA avec moins de gens, donc plus de TACE",
  "Des motivés pour faire Paris-Fontainebleau en trottinette ?",
  //RAF
  "Ah mais moi, j'aime bien quand les mecs viennent me voir.",
  "Ooooh, trop mignons vos messages !",
  "Nan mais moi ...",
  "Il etait quand même archi beau gosse !",
  //MBI
  "Senior Manager de mon cul !",
  "Des shots !",
  "Tournée de shots !",
  //JUJU
  "Oh l'enfer !",
  "Nan mais ce mec ...",
  //FAM
  "Girl Power !",
  "Team Laboure !",
  //MEG
  "Je suis à Creteil Soleil ce WE.",
  //"Et ouais, je bouffe comme une chienne !",
  //LEO
  "Y a que moi qui ait envie de ken avec cette chaleur ?",
  //CEM
  "Toi je t'aime bien !",
  //SLU
  "J'arrive dans 1h avec des tréteaux",
  "Qui est dispo pour un déménagement, j'offre les bières.",
  //CAN
  "Ah mais moi, je vais sur Grinder et je trouve direct !",
  //JORO
  //MID
  "Quand je suis triste, j'ai besoin de boire."
]